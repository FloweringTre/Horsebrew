...
func _on_interact():
	...
	velocity = Vector2.ZERO
	if picker == 1:
		Dialogic.start("res://Dialogue/HerdDialogue3.dtl")
		await Dialogic.timeline_ended
		timer.start(idle_time)
		return
	if picker == 2:
		Dialogic.start("res://Dialogue/HerdDialogue4.dtl")
		await Dialogic.timeline_ended
		timer.start(idle_time)
		return
	if picker == 3:
		Dialogic.start("res://Dialogue/HerdDialogue5.dtl")
		await Dialogic.timeline_ended
		timer.start(idle_time)
		return
	if picker == 4:
		Dialogic.start("res://Dialogue/HerdDialogue6.dtl")
		await Dialogic.timeline_ended
		timer.start(idle_time)
		return
	if picker == 5:
		Dialogic.start("res://Dialogue/HerdDialogue7.dtl")
		await Dialogic.timeline_ended
		timer.start(idle_time)
		return
	if picker == 0:
		Dialogic.start("res://Dialogue/HerdDialogue8.dtl")
		await Dialogic.timeline_ended
		timer.start(idle_time)
		return
