...
func DialogicSignalEvent(argument:String):
	if argument == "BrunoMain":
		Bfirst_dialogue = false
...
func _on_interact():
	...
	if Bfirst_dialogue == true:
		current_state = HORSE_STATE.IDLE
		state_machine.travel("idle_right")
		velocity = Vector2.ZERO
		Dialogic.start("res://Dialogue/HerdDialogue2.dtl")
		await Dialogic.timeline_ended
		timer.start(idle_time)
		return
	if Bfirst_dialogue == false:
		current_state = HORSE_STATE.IDLE
		state_machine.travel("idle_right")
		velocity = Vector2.ZERO
		Dialogic.start("res://Dialogue/HerdDialogue2a.dtl")
		await Dialogic.timeline_ended
		timer.start(idle_time)
		return
